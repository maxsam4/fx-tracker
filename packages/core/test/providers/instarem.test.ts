...
describe('instaremProvider.fetchQuote', () => {
  ...
  it('parses a successful Instarem direct response', async () => {
    installFetchMock({
      'https://www.instarem.com/api/v1/public/transaction/computed-rate': { body: success },
    });
    ...
    expect(single.receiveAmount).toBe(94500);
  });

  it('falls back to Wise comparison when Instarem returns 401', async () => {
    installFetchMock({
      'https://www.instarem.com/api/v1/public/transaction/computed-rate': {
        status: 401,
  ...
  it('throws when both Instarem and Wise comparisons fail', async () => {
    installFetchMock({
      'https://www.instarem.com/api/v1/public/transaction/computed-rate': { status: 500 },
  ...
  it('throws if Instarem direct response is missing fields', async () => {
    installFetchMock({
      'https://www.instarem.com/api/v1/public/transaction/computed-rate': {
        body: { source_amount: 1000 }, // missing fx_rate, destination_amount
      },
      'https://api.wise.com/v3/comparisons/': {
        body: { sourceCurrency: 'USD', targetCurrency: 'INR', sourceAmount: 1000, providers: [] },
      },
