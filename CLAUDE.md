...
- **Remitfinder is API-backed (signed-header aggregator).** Endpoint: `GET https://www.remitfinder.com/api/v1/rates/<ISO3-from>/<ISO3-to>` (USA/IND, ARE/IND). Auth headers `ai='WBST'`, `av='1.0.1'`, `ts=Date.now()`, and `as=base64(ascii(sha256_hex(path + ts + at)))` where `path` is the request path *without* the `/api/` prefix and `at` is a static base64 secret pulled from the SPA's environment config (currently `A5zlaaXHWzABnlO4I9XLzh4r4e6z3SB6NbouVaw0aVg=`). USD-INR is loose enough to accept missing headers, but AED-INR strictly enforces the signature — sign every request. Response shape: `remitRateResultDTOs[].latestRemitRate.tiers[]`, where each tier has `[lower, upper]` send-amount bounds. Pick the tier covering `sendAmount`; if every tier caps below, drop the provider for that poll. The plugin emits ONLY allowlisted brands (Ria/MoneyGram/WorldRemit/OFX/XE/CurrencyFair/PandaRemit/Revolut). Wise/Remitly/Instarem/WesternUnion/Aspora are intentionally NOT in the allowlist — Remitfinder publishes Remitly's promo rate and we never want that path. If `at` ever rotates, regrep `chunk-*.js` for `rf:{ai:` and update the constants.
